...
func TestFleetRequestJSON_Discover(t *testing.T) {
	req := FleetRequest{Type: "Discover"}
	assertJSON(t, req, `{"type":"Discover"}`)
}

func TestFleetRequestJSON_Launch(t *testing.T) {
	req := FleetRequest{
		Type:       "Launch",
		Command:    []string{"bash", "-l"},
		WorkingDir: "/home/user",
	}
	assertJSON(t, req, `{"type":"Launch","command":["bash","-l"],"working_dir":"/home/user"}`)
}

func TestFleetRequestJSON_Kill(t *testing.T) {
	req := FleetRequest{Type: "Kill", ID: uint32Ptr(5)}
	assertJSON(t, req, `{"type":"Kill","id":5}`)
}

func TestFleetResponseJSON_NodeInfo(t *testing.T) {
	url := "https://node1.example.com"
	resp := NewFleetResponseNodeInfo(NodeInfo{
		Name:        "node-1",
		ExternalURL: &url,
		Sessions:    []SessionInfo{},
		UptimeSecs:  3600,
	...
	if m["type"] != "NodeInfo" {
		t.Errorf("type = %v, want NodeInfo", m["type"])
	}
	if m["name"] != "node-1" {
		t.Errorf("name = %v, want node-1", m["name"])
	}
	if m["external_url"] != "https://node1.example.com" {
		t.Errorf("external_url = %v, want https://node1.example.com", m["external_url"])
	}
	if m["uptime_secs"].(float64) != 3600 {
		t.Errorf("uptime_secs = %v, want 3600", m["uptime_secs"])
	}
}

func TestFleetResponseJSON_SessionList(t *testing.T) {
	sessions := []SessionInfo{}
	resp := FleetResponse{
		Type:     "SessionList",
		Node:     "node-1",
	...
	assertJSON(t, resp, `{"type":"SessionList","node":"node-1","sessions":[]}`)
}

func TestFleetResponseJSON_Launched(t *testing.T) {
	resp := FleetResponse{
		Type: "Launched",
		Node: "node-1",
		ID:   uint32Ptr(42),
	}
	assertJSON(t, resp, `{"type":"Launched","node":"node-1","id":42}`)
}

func TestFleetResponseJSON_Error(t *testing.T) {
	resp := FleetResponse{
		Type:    "Error",
		Node:    "node-2",
		Message: "session not found",
	}
	assertJSON(t, resp, `{"type":"Error","node":"node-2","message":"session not found"}`)
}
